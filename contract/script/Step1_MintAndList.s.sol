...
contract Step1_MintAndList is Script {
    ...
    function run() external {
        ...
        // 1. 批量铸造 10 个 NFT
        TestNFT(NFT_ADDR).batchMint(100);
        ...
        for (uint256 i = 11; i <= 100; i++) {
            NFTTradeMarket(MARKET_ADDR).listNFT(NFT_ADDR, i, LIST_PRICE);
            console.log(" List token %d done", i);
        }

        // 4. 修改 token1 挂单价格为 0.15 ETH
        NFTTradeMarket(MARKET_ADDR).editListOrder(NFT_ADDR, 18, NEW_PRICE);
